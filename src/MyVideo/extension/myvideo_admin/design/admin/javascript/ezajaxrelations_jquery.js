jQuery(function( $ )
{
    ...
    function _addObject( link, boxID, id, name, className, sectionName, publishedTxt )
    {
        ...
        renderEditLink(tr, id);
    }

    function renderEditLink(tr, id){
        var link = tr.find('td:last-child a'), linkArray, renderedLink = '';
        linkArray = link.attr('href').split('/');
        linkArray[3] = id;
        for(var i = 0; i < linkArray.length; i++){
            if(i != 0)
                renderedLink += '/' + linkArray[i];
        }
        link.attr('href', renderedLink);
    }
