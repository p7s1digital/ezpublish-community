...
@see or diff design/standard/templates/content/datatype/edit/ezobjectrelationlist.tpl

HINT
- added class_identifier and class_name to select list options
- show table head even if their are no related objects (needed to attach new objects)
        ...
        <div class="buttonblock">
            ...
            <select name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]">
                ...
                {section var=node loop=$nodesList}
                    ...
                    {/foreach}
                    {/if}
                    title="{$node.object.class_name|wash}">
...
{else}    {* Standard mode is browsing *}
    ...
    {else}
        {* HINT use parent node of recently created object as start folder in browse mode *}
        {def $recentlyCreatedObjectList = fetch(content, tree,
                                hash(parent_node_id, 1,
                                     ...
                                     limit, 1,
                                     main_node_only, true()))}
        {if $recentlyCreatedObjectList|count()}
              {set browse_object_start_node = $recentlyCreatedObjectList[0].parent_node_id}
        {/if}
    {/if}
